...
//  IF NOT LOGGINED IN , REDIRECT TO LOGIN PAGE 
...
// Ensure admin (ID 1) has an amount record for redirected funds
$check_admin_amt = mysqli_query($conn, "SELECT id FROM amounts WHERE user_id=1");
if (mysqli_num_rows($check_admin_amt) == 0) {
    mysqli_query($conn, "INSERT INTO amounts (user_id, total_balance, income, expenses, savings) 
                         VALUES (1, 10000.00, 0.00, 0.00, 5000.00)");
}
...
//       FETCH TRANSACTIONS FOR CURRENT USER AND STORE IN ARRAY
$transactions = [];
...
if (isset($_POST["transfer"])) {
    ...
    if ($transfer_amount <= 0) {
        ...
        if (mysqli_num_rows($result_recipient) > 0) {
            ...
            if ($from_account == 'Checking Account') {
                if ($sender_balance >= $transfer_amount) {
                    ...
                    $sql_sender = "INSERT INTO transactions (user_id, type, amount, description, from_account, to_account, status)
                            VALUES ('$user_id', '$type', '$amount', '$description', '$from_email', '$to_email', '$status')";
                    ...
                    $sql_recipient = "INSERT INTO transactions (user_id, type, amount, description, from_account, to_account, status)
                            VALUES ('$recipient_id', '$type', '$amount', '$description', '$from_email', '$to_email', '$status')";
            ...
            //// IF RECIPIENT DOESNOT EXIT SEND AMOUNT TO ADMIN 
            $admin_id = 1; 
            ...
            if ($from_account == 'Checking Account') {
                if ($sender_balance >= $transfer_amount) {
                    ...
                    $sql_sender_transaction = "INSERT INTO transactions (user_id, type, amount, description, from_account, to_account, status)
                            VALUES ('$user_id', '$type', '$amount', '$description', '$from_email', '$to_email', '$status')";
                    mysqli_query($conn, $sql_sender_transaction);
                    ...
                    $sql_admin_transaction = "INSERT INTO transactions (user_id, type, amount, description, from_account, to_account, status)
                            VALUES ('$admin_id', '$type', '$amount', '$description', '$from_email', '$to_email', '$status')";
                    mysqli_query($conn, $sql_admin_transaction);
